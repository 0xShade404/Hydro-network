 ...
 *
 * Tests pair a real HydroToken (not a mock) against MockUSDH, copied in
 * the same way contracts/staking copies it. See
 * contracts/staking/scripts/compile.ts.
 */

const DEFI_DIR = path.join(__dirname, "..");
const CONTRACTS_DIR = path.join(DEFI_DIR, "contracts");
const ARTIFACTS_DIR = path.join(DEFI_DIR, "artifacts");
const HYDRO_TOKEN_SOURCE_PATH = path.join(DEFI_DIR, "..", "..", "contracts", "token", "contracts", "HydroToken.sol");
