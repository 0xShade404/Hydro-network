...
/**
 * Compiles contracts/*.sol with the npm-installed `solc` package — same
 * reason as contracts/token: Hardhat's own compiler downloader is blocked
 * by this environment's network policy.
 *
 * Tests pay rewards in real HydroToken, copied in the same way
 * contracts/staking copies it. See contracts/staking/scripts/compile.ts.
 */

const DEPIN_DIR = path.join(__dirname, "..");
const CONTRACTS_DIR = path.join(DEPIN_DIR, "contracts");
const ARTIFACTS_DIR = path.join(DEPIN_DIR, "artifacts");
const HYDRO_TOKEN_SOURCE_PATH = path.join(DEPIN_DIR, "..", "..", "contracts", "token", "contracts", "HydroToken.sol");
